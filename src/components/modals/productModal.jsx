...
export default function ProductModal({ show, onHide }) {
  ...
  const esDecimalValido = (val) =>
    val === "" || (val.includes(".") && !val.endsWith("."));
  ...
  const onSubmitForm = async (e) => {
    e.preventDefault();
    const respuesta = await handleSubmitProducto(e);

    if (respuesta?.ok) {
      onHide();
      setModalTitle(
        respuesta.esEdicion ? "PRODUCTO ACTUALIZADO" : "PRODUCTO GUARDADO",
      );
      setModalMessage(respuesta.mensaje);
  ...
  const handleCrearCategoria = async (e) => {
    ...
    if (nuevaCatInput.trim() !== "") {
      const resp = await crearNuevaCategoria(nuevaCatInput);
      setNuevaCatInput("");
      setModalTitle(resp.ok ? "CATEGORÍA CREADA" : "ATENCIÓN");
      setModalMessage(resp.mensaje);
      setShowSuccessModal(true);
    }
  };
    ...
    <>
      <Modal show={show} onHide={onHide} size="lg" backdrop="static">
        ...
        <Modal.Body id="cargarProducto_container">
          <Form onSubmit={onSubmitForm} id="cargarProducto_form">
            <Form.Group
              className="formGroup"
              controlId="formGroupCodigoProducto"
            >
            ...
            <Form.Group
              className="formGroup"
              controlId="formGroupNombreProducto"
            >
            ...
            <Form.Group
              className="formGroup"
              controlId="formGroupPrecioUnitario"
            >
